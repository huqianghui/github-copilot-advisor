...
### 1.3 GitHub live search 返回 422 —— 已实证

query 把中文关键词与 5 个 `repo:` 限定全塞进一条 `q=`,超出 GitHub search 的限制。

**影响**:`search_solutions` 的另一半也是死的。两个后端同时挂,模型只能靠
`web_search` 无 grounding 作答。

**修法**:未设计。需要决定是缩短 query、拆分多次请求、还是减少 `repo:` 限定。
...
### 4.5 两张 eval fixture 缺失 —— 已实证,**已重新评估为低优先级**
...
**只有一种情况值得回头补**:冒烟发现模型确实读不清小字 —— 那时 fixture 的作用
变成「钉住修复」(改 `detail: "high"` 或加放大预处理),那才是回归测试该守的东西。
