...
**规则 9(图片处理)**:收到图片时,**回答里**先用 1-2 句复述图中关键信息
(错误原文 / 配置项 / 界面位置),再走正常升级瀑布。
**把图中读到的错误文本作为 `search_solutions` 的 query 主体**,
不要用"用户发了一张截图"这类空泛 query。

"先"的作用域必须限定在**回答文本内部**,并显式声明不改变工具调用顺序。
否则模型可能把"先复述、再走瀑布"读成**轮次**顺序 —— 先回一句复述、
下一轮才检索 —— 那会直接违反规则 1 的"回答任何 Copilot 问题前,
必须先调用 search_solutions"。规则之间的优先关系要写明,不能留白让模型推断。
...
- **不支持计费/用量截图**:见 §1,需与 `copilot_usage_lookup` 的隐私规则
  一并评估后再开
- **纯图片消息的回答语言未定义**(2026-09-05 由 eval 用例暴露):
  主 spec 决策 9 规定"回答语言跟随提问语言",但纯图片消息**没有提问语言**。
  首轮纯图片时既无文本也无历史可依,模型实际会跟随图中文字的语言 ——
  而报错截图与 `settings.json` 截图几乎必然是英文,于是中文客户发一张英文
  截图会收到英文回答。

  本期**不处理**,理由:多轮场景下历史提供了语言信号,首轮纯图片是少数;
  且贸然规定"纯图片一律中文"会与决策 9 打架,需要产品侧先定调。
  `image-config-screenshot-no-text` 这条 eval 用例因此**刻意不设**
  `reply_language` —— 断言一个未定义的属性只会把正确回答判失败。

  若要处理,方向是给 prompt 规则 9 补一句无语言信号时的默认语言,
  并同步修改决策 9 的措辞。
